...
TITLE C0nw0nk - Directory Largest File first VP9
...
if not exist "%root_path:"=%win-x64\Tools\FfMpeg\bin\ffmpeg.exe" (
	call "%root_path:"=%PlexCleaner_VP9.cmd" "" "0" "0" "0"
)

:: start /b /wait "%root_path:"=%PlexCleaner_265.cmd" "%plex_folder:"=%\%%i" "0" "0" "0"
for /f "delims=" %%i in ('Type "%root_path:"=%%~n0.txt"') do (
	echo "%%i"
	for /f %%b in ('%root_path:"=%win-x64\Tools\FfMpeg\bin\ffmpeg.exe -i "%%i" 2^>^&1 ^| find /c /i "Video: vp9 "') do (
		if [%%b]==[0] (
			echo "%%i" is not VP9 encoded
			call "%root_path:"=%PlexCleaner_VP9.cmd" "%%i" "0" "0" "0"
		) else (
			echo "%%i" is VP9 encoded
		)
	)
)
